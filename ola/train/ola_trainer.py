...
class OlaTrainer(Trainer):
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        if getattr(self.args, 'tune_speech_adapter', False):
            ...
            super(OlaTrainer, self)._save_checkpoint(model, trial, metrics)

    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        if getattr(self.args, 'tune_mm_mlp_adapter', False):
            pass
        else:
            super(OlaTrainer, self)._save(output_dir, state_dict)