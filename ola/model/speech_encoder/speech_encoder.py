...
class DualWrappedEncoder(nn.Module):
    ...
    def load_beats(cls, model_config):
        ...
        beats_ckpt = torch.load(beats_path, map_location='cuda')
    ...
    def forward(self, x, raw_wav=None, audio_padding_mask=None):
        with torch.no_grad():
            ...
            audio_embeds = self.beats_model.extract_features(raw_wav.float(), padding_mask=audio_padding_mask, feature_only=True)[0].to(x.dtype)
